...
void test_parse_nmea_full()
{
    Serial1.clear();
    Serial1.feed("\r\n");
    Serial1.feed(gpsStream);

    TEST_ASSERT_TRUE(Serial1.available() > 0);
    TEST_ASSERT_TRUE(millis() >= 0);
    TEST_ASSERT_TRUE(Serial.println("Teste") >= 0);
    ...
    TEST_ASSERT_EQUAL_INT(51, timeinfo.tm_min);
    TEST_ASSERT_EQUAL_INT(3, timeinfo.tm_sec);
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.0001, 30.23664, lat);
    TEST_ASSERT_FLOAT_WITHIN(0.0001, -97.82145333, lng);

    int32_t latInt = GPS.getLatitudeInt();
    int32_t lngInt = GPS.getLongitudeInt();
    TEST_ASSERT_EQUAL_INT32(302366400, latInt);
    TEST_ASSERT_EQUAL_INT32(-978214533, lngInt);
}

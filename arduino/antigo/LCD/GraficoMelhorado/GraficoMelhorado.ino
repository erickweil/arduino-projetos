...
// NColunas     , Colincrem, ValorDoUltimo para aumentar 
    5           , 1        ,    0,// instantaneo
   (MAXRES-5)/6  ,10       ,    0,// passos de 2 seg
...
#define POS_X 0
#define POS_Y BOT_H*2
#define D (240-POS_Y)/4
...
//X MSB   ,X LSB    ,Y MSB    ,Y LSB   ,Width     ,Height       ,chars 0-8                                  ,quetela          ,EstiloDobotao  ,Pai (NBotoes se não tiver), acao(0 se nao tiver)
  ...
  0       ,Menu_W    ,0        ,0       ,Menu_W     ,BOT_H        ,'J','a','n','e','l','a','\0','\0','\0'    ,TELA_GLOBAL   ,BOT_BarraMenus  ,NBotoes,BOT_ACAO_SemAcao,//5
  ...
  0       ,Menu_W*2 ,0        ,0       ,Menu_W     ,BOT_H        ,'G','r','a','f','.','\0','\0','\0','\0'    ,TELA_GLOBAL   ,BOT_BarraMenus  ,NBotoes,BOT_ACAO_SemAcao,//9
  ...
  0       ,Menu_W*3 ,0        ,0       ,Menu_W     ,BOT_H        ,'A','j','u','d','a','\0','\0','\0','\0'    ,TELA_GLOBAL   ,BOT_BarraMenus  ,NBotoes,TELA_Ajuda,
    ...
    0     ,0    ,0        ,BOT_H ,320/3  ,BOT_H    ,'E','x','i','b','i','c','a','o','\0',TELA_Opcoes,BOT_BarraMenus,NBotoes   ,BOT_ACAO_SemAcao,    //22
    
    
    
    0     ,POS_X+D*2+20 +D -60    ,0        ,60+BOT_H   ,BOT_W  ,BOT_H    ,'T','e','m','p','\0','\0','\0','\0','\0',TELA_Opcoes,BOT_Texto,22   ,BOT_ACAO_Apenasfundo,
    0     ,POS_X+D*2+20 +D -60    ,0        ,70+BOT_H*2 ,BOT_W  ,BOT_H    ,'S','a','l','i','\0','\0','\0','\0','\0',TELA_Opcoes,BOT_Texto,22   ,BOT_ACAO_Apenasfundo, 
    0     ,POS_X+D*2+20 +D -60    ,0        ,80+BOT_H*3 ,BOT_W  ,BOT_H    ,'T','u','r','b','\0','\0','\0','\0','\0',TELA_Opcoes,BOT_Texto,22   ,BOT_ACAO_Apenasfundo,

    0     ,POS_X+D*2+30 +D      ,0        ,60+BOT_H   ,70  ,BOT_H    ,'d','e','\0','\0','\0','\0','\0','<','C',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum,
    0     ,POS_X+D*2+30 +D      ,0        ,70+BOT_H*2 ,70  ,BOT_H    ,'d','e','\0','\0','\0','\0','\0','<','S',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum, 
    0     ,POS_X+D*2+30 +D      ,0        ,80+BOT_H*3 ,70  ,BOT_H    ,'d','e','\0','\0','\0','\0','\0','<','T',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum,    
    0     ,POS_X+D*2+30+75+D    ,0        ,60+BOT_H   ,70  ,BOT_H    ,'a','t','e','\0','\0','\0','\0','>','C',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum,
    0     ,POS_X+D*2+30+75+D    ,0        ,70+BOT_H*2 ,70  ,BOT_H    ,'a','t','e','\0','\0','\0','\0','>','S',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum, 
    0     ,POS_X+D*2+30+75+D    ,0        ,80+BOT_H*3 ,70  ,BOT_H    ,'a','t','e','\0','\0','\0','\0','>','T',TELA_Opcoes,BOT_InputNum,22   ,BOT_ACAO_InputNum,
    
    0     ,320/3     ,0        ,BOT_H ,320/3  ,BOT_H    ,'C','a','l','i','b','r','a','r','\0',TELA_Opcoes,BOT_BarraMenus,NBotoes   ,BOT_ACAO_SemAcao,    //32
...
//*************** possiveis Estados *****************
/* 
0 - nao esta desenhado
1 - desenhado, mas nao apertado
2 - desenhado e apertado
3 - não esta desenhado, mas esta apertado
...
int Temperatura [MAXRES*DiviDeTempo] ={}; 
int Umidade     [MAXRES*DiviDeTempo] ={};     
int Turbidez    [MAXRES*DiviDeTempo] ={};  
int Lei[NSENSORES]={};
boolean des=true;
...
void setup(void) 
{
Estado[NBotoes] = true;  // diz que os botoes filhos do global devem aparecer
...
Leitura = constrain(Leitura, Get(Leituras,LESTRIP*0+0),Get(Leituras,LESTRIP*0+1));
...
Leitura = constrain(Leitura, Get(Leituras,LESTRIP*1+0),Get(Leituras,LESTRIP*1+1));
...
Leitura = constrain(Leitura, Get(Leituras,LESTRIP*2+0),Get(Leituras,LESTRIP*2+1));
...
{  
  ...
  QuantasVezes = constrain(QuantasVezes,1,MAXRES-1);
...
algoclicado = true;
  if(x>px && y>py && x<px+width && y<py+height)
  {
    ...
    ApagarSubMenus(i,Get(botoes,BSTRIP*i+16));
    break;
  }
}
}
...
{
if(estilo == BOT_InputNum)
{
filhodequem = NBotoes+1;
...
{
byte at=0;  
byte umbyte =Texto.charAt(at++); // Lê o prox byte na porta serial
String num="";
if(umbyte == '-')
{
num += (char)umbyte;
...
while(isDigit(umbyte)) // enquanto os bytes que forem lidos ainda forem digitos ou um ponto
{
...
String txt="";
for(int i=6;i<15,(char)Get(botoes,BSTRIP*index+i)!='\0';i++)
txt +=(char)Get(botoes,BSTRIP*index+i);
switch(Get(botoes,BSTRIP*index+16))
{
 ...
 case BOT_BarraMenus:// ****************************************
  ...
  tft.setTextColor(BLACK);
  switch((char)Get(botoes,BSTRIP*index+14))
  {
  case 'C':
  if((char)Get(botoes,BSTRIP*index+13)== '<')
...
byte Remapear(float m)
{
m = mapfloat(m, Get(Leituras,LESTRIP*qual+0),Get(Leituras,LESTRIP*qual+1),Get(Leituras,LESTRIP*qual+2),Get(Leituras,LESTRIP*qual+3));
if(m<Leituras2[LESTRIP*qual+2])
m = Leituras2[LESTRIP*qual+2];
if(m>Leituras2[LESTRIP*qual+3])
m = Leituras2[LESTRIP*qual+3];
//m = constrain(m,LESTRIP*qual+2,LESTRIP*qual+3);
m = mapfloat(m,Leituras2[LESTRIP*qual+2],Leituras2[LESTRIP*qual+3],YMIN,YMAX);
m = constrain(m,YMIN,YMAX);
m = YMAX-m+YMIN; 
return (byte)m;
}
void desenharGrafico(int grafico[])//************************ DESENHA UM GRAFICO DE ACORDO COM O ARRAY DADO
{
  ...
  for(byte i=MAXRES-5;i>0;i--)
  {
    int x = W-(larg*i);
    int x2 = W-(larg*(i+1));
    byte y  = Remapear(grafico[(MAXRES*resolucao)+i]);
    byte y2 = Remapear(grafico[(MAXRES*resolucao)+i+1]);
    byte y3 = Remapear(grafico[(MAXRES*resolucao)+i+2]);
...
tft.setTextColor(BLACK);
  //        Medição
tft.setCursor(20,YMIN);
switch(qual)
{
 case 0:tft.print(F("Graus Celcius"));break; 
 case 1:tft.print(F("Miligramas por Litro"));break; 
 case 2:tft.print(F("NTU"));break; 
...
for(i=Get(Leituras2,LESTRIP*qual+4)-1;i>=0;i--)
{
...
inc=0;
//          tempo
tft.setCursor(W-60,H-65);
switch(escala)
{
 case 0:tft.print(F("Segundos"));break; 
 case 1:tft.print(F("Segundos"));break; 
 case 2:tft.print(F("Minutos"));break; 
 case 3:tft.print(F("Minutos"));break; 
 case 4:tft.print(F("Horas"));break; 
}
for(i=Get(Escalas,ESCSTRIP*escala);i>0;i--)
{
...
void empurrar(int graf[],byte de,byte ate,int Leitura) // todos os elementos do array graf é empurrado em 1, e o index "de" recebe a Leitura, "ate" foi empurrado para fora
{ 
 int temp=0;
 int temp2=Leitura;
...
int media(int graf[],byte de,byte ate)
{
long retorno=0;
byte q=0;
